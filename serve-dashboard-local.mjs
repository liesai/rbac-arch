...
async function serveFile(req, res) {
  ...
  if (
    reqPath === '/group-config.html' ||
    reqPath === '/blueprint' ||
    reqPath === '/policy-admin.html' ||
    reqPath === '/group-designer.html'
  ) {
    const page = reqPath === '/blueprint' ? 'group-config.html' : reqPath.slice(1);
    const filePath = path.join(ROOT_DIR, page);
    try {
      const body = await readFile(filePath);
      send(res, 200, body, withSecurityHeaders({ 'Content-Type': MIME['.html'] }));
    } catch {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('Blueprint page: /group-config.html');
});
